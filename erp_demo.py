# -*- coding: utf-8 -*-
"""Al-Ghannam Fabrics ERP & CRM | Master Edition"""
import base64
import os
import re
import sqlite3
...
# ==========================================
# ⚙️ إعدادات البيزنس الأساسية (الغنام لتجارة الأقمشة)
# ==========================================
DB_NAME = os.environ.get("ALGHANNAM_DB_PATH", "Algannam_Trading.db") 
DEFAULT_COMPANY = "شركة الغنام لتجارة الأقمشة" 
TAGLINE = "نظام تخطيط الموارد وإدارة تجارة الأقمشة (جملة وقطاعي)"
PAY_METHODS = ["كاش", "تحويل بنكي", "محفظة", "آجل", "شيك"]
# تم إضافة "قماش خام" لتشغيل ورشة القص والتجهيز
ITEM_TYPES = ["سادة بورسل مفتوح", "مطبوع بورسل مفتوح", "كوفرتة", "سادة مقفول", "مطبوع بورسل مقفول", "قماش خام", "إكسسوارات"] 
PRICING_TIERS = ["قطاعي", "جملة", "موزعين / مصانع"]
# ==========================================
...
st.set_page_config(page_title=f"{DEFAULT_COMPANY} ERP", page_icon="🧵", layout="wide", initial_sidebar_state="expanded")
...
# تهيئة المتغيرات والجلسات وقاعدة البيانات
if 'logged_in' not in st.session_state: st.session_state['logged_in'] = False
if 'role' not in st.session_state: st.session_state['role'] = None
if 'username' not in st.session_state: st.session_state['username'] = None
if 'cart' not in st.session_state: st.session_state['cart'] = []
if 'kc_cart' not in st.session_state: st.session_state['kc_cart'] = []
if 'nav_menu' not in st.session_state: st.session_state['nav_menu'] = "🏠 الرئيسية (الداشبورد)"
...
def init_db():
    with get_conn() as conn:
        ...
        c.execute("INSERT OR IGNORE INTO settings (id, company_name, phone, address) VALUES (1,?,?,?)", (DEFAULT_COMPANY, "01000000000", "مصر"))
        c.execute("INSERT OR IGNORE INTO users (username, password, role) VALUES ('admin', '123', 'مدير')")
        c.execute("INSERT OR IGNORE INTO platforms (name) VALUES ('أمازون'), ('نون'), ('المتجر الإلكتروني')")
...
# ==========================================
# 1. الدوال المساعدة وتصميم الـ HTML والـ CSS الشامل
# ==========================================
def get_settings():
...
def load_logo_html():
    ...
    return f'<div style="text-align:center; color:#c19b62; font-weight:bold; font-size:32px; letter-spacing: 2px;">{DEFAULT_COMPANY}</div>'
...
def apply_theme():
    ...
    .stApp {{ background-color: #0a110d !important; color: #ece6da !important; }}
    [data-testid="stSidebar"] {{ background-color: #111d17 !important; border-left: 1px solid #1a5238; }}
    [data-testid="stHeader"] {{ background: rgba(10, 17, 13, 0.9) !important; }}
    ...
    .lux-metric {{ background: #111d17; border: 1px solid #c19b62; border-radius: 10px; padding: 15px; text-align: center; margin-bottom: 15px; }}
    .lux-metric .lbl {{ color: #a89a82; font-size: 14px; font-weight: bold; }}
    .lux-metric .val {{ color: #c19b62; font-size: 26px; font-weight: 900; margin-top: 5px; }}
    .lux-metric.danger {{ border-color: #d32f2f; }}
    .lux-metric.danger .val {{ color: #ff5252; }}
    .stButton>button {{ background: #143d2a !important; color: #c19b62 !important; border: 1px solid #c19b62 !important; border-radius: 8px !important; transition: 0.3s; font-weight: bold !important;}}
    .stButton>button:hover {{ background: #c19b62 !important; color: #143d2a !important; border-color: #fff !important; transform: scale(1.02); }}
    .home-btn>button {{ background: #c19b62 !important; color: #143d2a !important; font-size: 18px !important; border: 2px solid #fff !important; margin-bottom: 20px; }}
    .home-btn>button:hover {{ background: #fff !important; color: #143d2a !important; }}
...
if not st.session_state.logged_in:
    ...
    with mid:
        st.markdown(f'<div class="main-header">{load_logo_html()}<br><h2 style="color:#c19b62;">تسجيل الدخول</h2></div>', unsafe_allow_html=True)
        with st.form("login"):
...
st.sidebar.markdown(f"<div style='text-align:center; padding:10px; background:#1a5238; border-radius:10px; color:#c19b62; margin-bottom:20px;'>مرحباً يا <b>{st.session_state.username}</b><br><small>({st.session_state.role})</small></div>", unsafe_allow_html=True)

def go_home(): st.session_state['nav_menu'] = "🏠 الرئيسية (الداشبورد)"
st.sidebar.markdown('<div class="home-btn">', unsafe_allow_html=True)
st.sidebar.button("🏠 العودة للرئيسية", on_click=go_home, use_container_width=True)
...
if st.session_state.role == "مدير":
    MENU = [
        ...
        "عروض الأسعار والكتالوج", "🏬 قسم مبيعات التجزئة (المعرض)", "✂️ ورشة القص والتجهيز", "📱 التسويق بالواتساب", 
        "🛠️ الإعدادات العامة", "👑 لوحة تحكم المدير"
    ]
else:
    MENU = [
        "🏠 الرئيسية (الداشبورد)", "🛒 المبيعات والمشتريات", "🏬 قسم مبيعات التجزئة (المعرض)"
    ]

choice = st.sidebar.radio("القائمة:", MENU, key="nav_menu")

st.sidebar.markdown("---")
if st.sidebar.button("🚪 تسجيل الخروج", key="btn_logout"): 
    st.session_state.logged_in = False
    st.session_state.role = None
    st.rerun()

st.markdown(f'<div class="main-header">{load_logo_html()}<div style="color:#a89a82; margin-top:10px;">{TAGLINE}</div></div>', unsafe_allow_html=True)
...
if choice == "🏠 الرئيسية (الداشبورد)":
    st.markdown("## 📊 نظرة عامة على البيزنس (لوحة القيادة)")
    with get_conn() as conn:
        ...
        lux_box(c3, "عدد الفواتير اليوم", f"{conn.execute('SELECT COUNT(*) FROM sales WHERE date=?', (now_dt()[0],)).fetchone()[0]}")
        
        st.markdown("---")
        if st.session_state.role == "مدير":
            show_inv = st.toggle("👁️ كشف/إخفاء تفاصيل وتكلفة المخزون (بيانات سرية)")
            if show_inv:
                inv_df = pd.read_sql_query("SELECT type, qty, buy_price FROM inv", conn)
                total_value = (inv_df['qty'] * inv_df['buy_price']).sum()
                
                st.markdown("### 📦 تفاصيل الجرد المباشر")
                i1, i2 = st.columns(2)
                lux_box(i1, "إجمالي كمية البضاعة في المخزن", f"{inv_df['qty'].sum():,.1f} وحدة")
                lux_box(i2, "التكلفة الإجمالية للمخزون", f"{total_value:,.2f} ج.م", "danger")

elif choice == "🛒 المبيعات والمشتريات":
    st.markdown("## 🛒 إدارة المبيعات والمشتريات")
    t_sale, t_buy, t_ret = st.tabs(["🛒 نقطة البيع (إصدار فاتورة)", "📥 إدخال مشتريات", "🔙 المرتجعات"])
    ...
    with t_sale:
        ...
        with cl:
            ...
            if client == "+ عميل جديد": client = st.text_input("اسم العميل الجديد")
            elif client != "عميل نقدي":
                c_info = c_df[c_df['name'] == client].iloc[0]
                c_tier = c_info['pricing_tier']
            ...
            if not items.empty:
                ...
                elif c_tier in ["موزعين", "موزعين / مصانع"] and i_row['dist_price'] > 0: def_p = i_row['dist_price']
                
                qty = st.number_input("الكمية", 0.1, value=1.0)
                up = st.number_input("السعر", 0.0, value=float(def_p))
                if st.button("➕ إضافة للفاتورة"): 
                    st.session_state.cart.append({"item": i_n, "type": i_t, "qty": qty, "p": up, "t": qty*up}); st.rerun()
    ...
    with get_conn() as conn:
        ...
        with t_purch_track:
            ...
            df_p = pd.read_sql_query("SELECT date 'التاريخ', time 'الوقت', supplier 'المورد', item 'الصنف', type 'النوع', qty 'الكمية', total 'الإجمالي' FROM purchases WHERE item!='سداد دفعة نقدية' AND supplier!='نقل داخلي' ORDER BY id DESC LIMIT 200", conn)
    ...
    with get_conn() as conn:
        with t_trs:
            ...
            if not bank_df.empty:
                ...
                else: st.info("لا توجد حركات بنكية مسجلة بتفاصيل.")
            else: st.info("لا توجد أرصدة بنكية حالياً.")
    ...
    with get_conn() as conn:
        with t_daily:
            ...
            day_purchases = conn.execute("SELECT COALESCE(SUM(total),0) FROM purchases WHERE date=? AND item!='سداد دفعة نقدية' AND supplier!='نقل داخلي'", (sel_date,)).fetchone()[0]
            ...
            lux_box(d3, "أموال محصلة (إيداع)", f"{day_in:,.2f} ج.م")
            lux_box(d4, "أموال مدفوعة (سحب)", f"{day_out:,.2f} ج.م", "danger")
    ...
    with get_conn() as conn:
        with col_todo:
            ...
            new_task = st.text_input("مهمة جديدة (مثال: كلم عميل فلان)")
        ...
        with col_crm:
            ...
            with st.expander("➕ إضافة عميل محتمل جديد (Lead)"):
                c1, c2 = st.columns(2)
                n_name = c1.text_input("اسم العميل / المصنع")
    ...
    with get_conn() as conn:
        ...
        b_choice = col1.radio("تخصيص الهوية (البراند):", ["بيزنس الجملة (الأساسي)", "بيزنس التجزئة (الفرعي)"])
        brand_key = "green" if "الجملة" in b_choice else "cup"
        ...
        def get_price(r):
            ...
            if tier_choice in ["موزعين", "موزعين / مصانع"] and r['dist_price'] > 0: return r['dist_price']
            return r['sell_price']
        ...
        with t_menu:
            c1, c2 = st.columns(2)
            menu_title = c1.text_input("عنوان المنيو (اتركه فارغاً للإلغاء):", "CATALOG")
            menu_sub = c2.text_input("النص الفرعي:", "قائمة أسعار التوريد والجملة")
            template_choice = st.selectbox("اختر قالب التصميم (A4):", [1, 2, 3], format_func=lambda x: {1: "1 — Classic Luxury (ذهبي مع هوية البراند)", 2: "2 — Minimalist Executive (أبيض وداكن - للطباعة العادية)", 3: "3 — Royal Emerald (زمردي ملكي - فخم جداً)"}[x])
...
elif choice == "🏬 قسم مبيعات التجزئة (المعرض)":
    st.markdown(f'<div style="background:linear-gradient(135deg, #143d2a, #1a5238);padding:25px;border-radius:12px;color:#fff;text-align:center;margin-bottom:25px;box-shadow:0 10px 20px rgba(0,0,0, 0.2);"> <h1 style="margin:0;font-size:42px;font-weight:900;letter-spacing:2px;color:#fff;">🏬 معرض التجزئة</h1><p style="margin:10px 0 0 0;font-size:18px;opacity:0.9;">إدارة المبيعات المباشرة، التجارة الإلكترونية، والمصروفات المستقلة</p></div>', unsafe_allow_html=True)
    
    with get_conn() as conn:
        t_pos, t_inv, t_ecom, t_kc_exp = st.tabs(["🛒 كاشير المعرض", "📦 مخزون ونقل داخلي", "🌐 التجارة الإلكترونية", "💸 مصروفات المعرض"])
        
        with t_kc_exp:
            st.markdown("### 💸 مصروفات تشغيل المعرض (تسويق، أكياس، أخرى)")
            st.info("سجل المبالغ المدفوعة لموردي التشغيل هنا ليتم خصمها من الخزينة وحسابها ضمن المصروفات.")
            
            ex_c1, ex_c2 = st.columns(2)
            ex_cat = ex_c1.selectbox("بند المصروفات:", ["ماركتينج وإعلانات", "أكياس وتغليف", "طباعة ورقيات", "أخرى"])
            ex_sup = ex_c2.text_input("الجهة المدفوع لها:")
            
            ex_amt = ex_c1.number_input("المبلغ المدفوع:", min_value=0.01)
            ex_pm = ex_c2.selectbox("وسيلة الدفع:", PAY_METHODS, key="ex_pm")
            ex_b = ex_c2.selectbox("من حساب:", banks_list, key="ex_b") if ex_pm in ["تحويل بنكي", "محفظة"] else "---"
            
            if st.button("💾 تسجيل مصروف التشغيل") and ex_sup:
                conn.execute("INSERT INTO purchases (date,time,supplier,item,type,qty,total,paid,pay_method) VALUES (?,?,?,?,?,?,?,?,?)", (now_dt()[0], now_dt()[1], ex_sup, f"مصروفات معرض: {ex_cat}", "-", 1, ex_amt, ex_amt, ex_pm))
                insert_treasury(conn, "سحب", ex_cat, f"دفع لـ {ex_sup}", ex_amt, ex_pm, ex_b)
                conn.commit(); st.success("تم تسجيل المصروفات بنجاح!"); st.rerun()

        with t_inv:
            retail_types = "('سادة بورسل مفتوح', 'مطبوع بورسل مفتوح', 'كوفرتة', 'سادة مقفول', 'مطبوع بورسل مقفول', 'إكسسوارات')"
            st.dataframe(pd.read_sql_query(f"SELECT name الصنف, type النوع, qty الكمية, sell_price السعر FROM inv WHERE type IN {retail_types}", conn), use_container_width=True)
            st.markdown("### 🔄 تحويل البضاعة بين الأقسام الداخلية")
            src_inv = pd.read_sql_query(f"SELECT name, type, qty, buy_price FROM inv WHERE qty > 0", conn)
            if not src_inv.empty:
                c1, c2 = st.columns(2)
                options_src = [f"{r['name']} | {r['type']}" for _, r in src_inv.iterrows()]
                sel_src = c1.selectbox("الصنف المحول من المخزن الرئيسي:", options_src)
                src_n, src_t = sel_src.split(" | ")
                src_row = src_inv[(src_inv['name']==src_n) & (src_inv['type']==src_t)].iloc[0]
                tgt_type = c2.selectbox("نوع الصنف بعد النقل:", [t for t in ITEM_TYPES if t != 'قماش خام'])
                tgt_name = c2.text_input("اسم الصنف بعد النقل:", value=src_n)
                qx = c1.number_input("الكمية المراد نقلها:", 0.1, value=1.0)
                ic = c2.number_input("تكلفة الوحدة (نقل داخلي):", 0.0, value=float(src_row['buy_price']))

                if st.button("📥 تأكيد النقل الداخلي"):
                    ...
                    conn.execute("INSERT INTO purchases (date,time,supplier,item,type,qty,total,paid,pay_method) VALUES (?,?, 'نقل داخلي', ?, ?, ?, ?, ?, 'داخلي')", (now_dt()[0], now_dt()[1], tgt_name, tgt_type, qx, qx*ic, qx*ic))
                    conn.commit()
                    st.success("تم النقل بنجاح!"); st.rerun()
        ...
        with t_ecom:
            ...
            with ec3:
                ...
                if st.button("➕ إضافة منصة") and new_plat:
                    try: conn.execute("INSERT INTO platforms (name) VALUES (?)", (new_plat,)); conn.commit(); st.success("تم الإضافة!"); st.rerun()
        ...
        with t_pos:
            ...
            cl, cr = st.columns([1, 1.2])
            with cl:
                client = st.text_input("اسم العميل (التجزئة):", value="عميل نقدي (معرض)")
                if not items.empty:
                    options = [f"{r['name']} | {r['type']}" for _, r in items.iterrows()]
                    ...
                    if st.button("➕ إضافة للسلة", key="kc_add"):
                        st.session_state['kc_cart'].append({"item": str(i_row['name']), "type": str(i_row['type']), "qty": qty, "p": up, "t": qty*up}); st.rerun()
                ...
                if st.session_state.get('kc_cart'):
                    ...
                    if st.button("🟧 إصدار فاتورة التجزئة"):
                        d, tm = now_dt()
                        inv_no = f"RT-{datetime.now().strftime('%M%S')}"
                        for i, ln in enumerate(st.session_state['kc_cart']):
                            ...
                            conn.execute("UPDATE inv SET qty=qty-? WHERE name=? AND type=?", (ln["qty"], ln["item"], ln["type"]))
                        if paid > 0 and pm != "آجل": insert_treasury(conn, "إيداع", "مبيعات معرض", f"إيصال {inv_no}", paid, pm, bank_ch)
                        conn.commit()
            ...
            with cr:
                if "kc_res" in st.session_state: show_capture_component(build_invoice_html(st.session_state.kc_res, get_settings()), f"Retail_Invoice_{st.session_state.kc_res['no']}")

elif choice == "✂️ ورشة القص والتجهيز":
    st.markdown("## ✂️ ورشة التجهيز والقص وحساب التكلفة")
    with get_conn() as conn:
        raw_mats = pd.read_sql_query("SELECT name, qty, buy_price, sell_price FROM inv WHERE type='قماش خام'", conn)
        if raw_mats.empty: st.warning("لا توجد أقمشة خام مسجلة في النظام. الرجاء الشراء أولاً كـ 'قماش خام'.")
        else:
            c1, c2 = st.columns(2)
            gn = c1.selectbox("الخامة الأساسية المراد قصها/تجهيزها:", raw_mats['name'].tolist())
            iw = c1.number_input("الكمية الداخلة للورشة (متر/ثوب)", min_value=0.1, value=10.0)
            loss = c1.number_input("نسبة الفاقد المتوقعة (هدر القصاقيص) %", 0.0, 50.0, 5.0)
            rn = c2.text_input("اسم المنتج النهائي المجهز:", value=f"{gn} جاهز")
            r_type = c2.selectbox("نوع المنتج النهائي:", [t for t in ITEM_TYPES if t != 'قماش خام'])
            
            g = raw_mats[raw_mats['name'] == gn].iloc[0]
            nq = iw * (1 - loss / 100)
            total_cost = iw * float(g['buy_price'])
            rc = total_cost / nq if nq > 0 else 0
            rs = rc * 1.3 
            
            c2.info(f"الصافي المتوقع: **{nq:,.2f} وحدة** | التكلفة الجديدة للوحدة: **{rc:,.2f} ج.م**")
            
            if st.button("✂️ تأكيد العملية وتحديث المخزن") and rn:
                conn.execute("UPDATE inv SET qty=qty-? WHERE name=? AND type='قماش خام'", (iw, gn))
                conn.execute("INSERT INTO inv (name,type,qty,buy_price,sell_price) VALUES (?,?,?,?,?) ON CONFLICT(name,type) DO UPDATE SET qty=qty+?,buy_price=?", (rn, r_type, nq, rc, rs, nq, rc))
                ...
                conn.commit(); st.success("تمت عملية القص والتجهيز وتحديث الأرصدة بنجاح!"); st.rerun()

elif choice == "📱 التسويق بالواتساب":
    st.markdown("## 📱 حملات التسويق المباشر (WhatsApp Marketing)")
    with get_conn() as conn:
    ...
    st.info(f"يوجد عدد **{len(tg)}** عميل مسجل بأرقام هواتف في النظام.")
    msg = st.text_area("نص الحملة أو العرض الترويجي:", f"عروض حصرية من {DEFAULT_COMPANY} 🧵\n\nاطلب الآن...", height=150)
...
elif choice == "🛠️ الإعدادات العامة":
    st.markdown("## 🛠️ إعدادات النظام المتقدمة")
    t1, t2, t3 = st.tabs(["الإعدادات و Gemini", "تعديل الأسعار والشرائح", "العملاء والائتمان"])
    with get_conn() as conn:
        with t1:
            sc = get_settings()
            cn = st.text_input("اسم الشركة المعروض في الفواتير", sc["company_name"])
            ph = st.text_input("هاتف التواصل", sc["phone"])
    ...
    st.markdown("## 👑 لوحة تحكم المدير (الإدارة الشاملة)")
    st.warning("⚠️ أي تعديل أو حذف هنا ينعكس مباشرة على قاعدة البيانات. يرجى توخي الحذر.")
    
    tables = {
        "👥 إدارة المستخدمين والصلاحيات (Users)": "users",
        "📦 المخزون (Inventory)": "inv",
    ...
    table_label = st.selectbox("اختر الجدول المراد إدارته:", list(tables.keys()))
    ...
    with get_conn() as conn:
        ...
        if table_name == "users":
            st.info("💡 **إدارة الصلاحيات:** استخدم عمود Role لتحديد دور المستخدم. (مدير) يرى كل شيء، و (كاشير) يرى المبيعات والتجزئة فقط.")
            col_cfg = {"role": st.column_config.SelectboxColumn("الصلاحية (Role)", options=["مدير", "كاشير"], required=True)}
        else:
            st.info("💡 يمكنك التعديل المباشر في أي خانة، أو تحديد صف والضغط على (Delete) لمسحه من الكيبورد.")
            col_cfg = {}
        ...
        if st.button("💾 تأكيد وحفظ التعديلات في قاعدة البيانات"):
            try:
                ...
                if deleted_ids:
                    for did in deleted_ids: cur.execute(f"DELETE FROM {table_name} WHERE id=?", (did,))
                        
                conn.commit(); st.success(f"✅ تم حفظ التعديلات بنجاح!"); st.rerun()
            except sqlite3.IntegrityError:
                st.error("❌ خطأ: هذا الاسم (أو القيمة الفريدة) مسجل مسبقاً. لا يمكن التكرار.")
